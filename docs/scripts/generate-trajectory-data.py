#!/usr/bin/env python3
"""Generate pre-parsed trajectory data (JSON snapshot + frames) for the gallery.

Outputs a JSON file with the same base snapshot fields as generate-demo-data.py,
plus a `frames` array containing per-frame positions for the caffeine+water vibration
trajectory.  The frame positions use the same deterministic formula as
tests/fixtures/generate_caffeine_water_xtc.py so no XTC parser is needed.

Requires: numpy (available via `uv run` or `pip install numpy`)
"""
...
# ─── PDB parsing + bond inference (same as generate-demo-data.py) ───────────
...
# ─── Trajectory frame generation ────────────────────────────────────────────

N_FRAMES = 100
AMPLITUDE = 0.3  # Angstroms


def generate_frames(base_positions_flat: list[float], n_atoms: int) -> list[dict]:
    """Generate vibration frames with the same formula as generate_caffeine_water_xtc.py."""
    base = np.array(base_positions_flat, dtype=np.float32).reshape(n_atoms, 3)
    rng = np.random.RandomState(42)
    phases = rng.uniform(0, 2 * np.pi, size=(n_atoms, 1))
    scale = rng.uniform(0.5, 1.0, size=(n_atoms, 3))
    frames = []
    for i in range(N_FRAMES):
        t = i / N_FRAMES * 2 * np.pi
        displacement = AMPLITUDE * np.sin(t + phases) * scale
        positions = (base + displacement.astype(np.float32)).flatten()
        frames.append({
...
def main():
    ...
    snapshot = parse_pdb(pdb_path.read_text())
    snapshot["frames"] = generate_frames(snapshot["positions"], snapshot["nAtoms"])

    out_path = out_dir / "caffeine_water_vibration.json"
    ...
    print(
        ...
        f"({snapshot['nAtoms']} atoms, {snapshot['nBonds']} bonds, "
        f"{N_FRAMES} frames, {size_kb:.1f} KB)"
    )
