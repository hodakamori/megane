/**
 * Cloudflare Worker that proxies pipeline-chat requests to OpenRouter's
 * free-tier models so the docs demo can offer AI chat without asking
 * visitors for their own API key.
 *
 * Responsibilities:
 *  - Restrict access to the configured demo origin (CORS + Origin check)
 *  - Apply a per-IP rate limit backed by Workers KV
 *  - Forward sanitized chat messages to OpenRouter with the server-held
 *    API key and stream the SSE response back untouched
 */
