...
describe("DrawingBoundaryNode", () => {
  ...
  it("exposes independent fractional min/max controls for a, b, and c", () => {
    ...
    expect(screen.getByLabelText("a minimum")).toHaveValue(0);
    expect(screen.getByLabelText("a maximum")).toHaveValue(1);
    expect(screen.getByLabelText("b minimum")).toHaveValue(0);
    expect(screen.getByLabelText("c maximum")).toHaveValue(1);
  });
  ...
  it("updates an arbitrary bound", () => {
    ...
    render(<DrawingBoundaryNode {...props()} />);
    fireEvent.change(screen.getByLabelText("a minimum"), { target: { value: "-0.2" } });
