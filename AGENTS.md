# megane - Agent Instructions (Claude Code, Codex CLI)
...
12. **Every LLM benchmark case is a prompt + JSON + image triple — never add one without all three.** `bench/llm/` grades megane's pipeline generator, and a case is only gradeable when the prompt, the pipeline that answers it, and the picture that pipeline draws live together and stay in sync. To add a case, write the prompt and rubric in `bench/llm/dataset.ts` **and** create `bench/llm/golden/<case id>/` holding all three: `pipeline.megane.json`, `expected.png`, and a `meta.json` naming the fixture, the expectation in prose, and where the pipeline came from. The folder name *is* the dataset case id — `bench/llm/golden.ts` throws on a folder that names no case, and `tests/ts/bench/golden.test.ts` fails if any of the three files is missing — so the pairing cannot silently rot. Four rules make the triple worth having: (a) **capture the JSON, never hand-author it** — build the pipeline in the editor and take `store.serialize()`; hand-written references have shipped bond queries megane rejects (`resname` is not a bond field) and node params that live on a different node type, and every one of them scored full marks against the static rubric while drawing nothing. (b) **Capture the PNG through `renderPipeline()` in `bench/llm/render.ts`** — the same path the bench grades a candidate through — and **look at it** before committing. (c) **Give every case a counterexample**: a named wrong pipeline that must NOT reproduce the image. An image comparison that accepts everything is indistinguishable from one that works, and `tests/e2e/bench-golden.spec.ts` asserts both directions for exactly that reason. (d) When the runner fails, **re-capture the pipeline, do not re-record the PNG** — a moved reference image hides the regression it was meant to catch. Cases whose picture provably cannot discriminate (the ephemeral data a `SerializedPipeline` does not carry — trajectory frames, volumetric grids, `fileVectors` — is dropped by `deserialize`, so `load_trajectory` and `load_vector` draw nothing after a round trip) must say so in `meta.json` rather than ship an image that grades nothing.
...
Full runbook including environment setup, per-host quirks, and matrix expansion lives in `.agents/skills/e2e-coverage/SKILL.md`.
...
| Single-feature projects   | `:format-loading`, `:playback`, `:sidebar`, `:licorice`, `:widget-api`, `:widget-examples`, `:pipeline-editor`, `:pipeline-file`, `:render-modal`, `:atom-bond-junction`, `:templates`, `:phase2` | Webapp host unless the project name encodes another                                                                                                                              |
...
Project-specific skills are defined in `.agents/skills/<name>/SKILL.md` (the vendor-neutral [Agent Skills](https://agentskills.io/specification) layout). Codex CLI discovers that directory natively; Claude Code reads the same files through the `.claude/skills` symlink. Each skill provides instructions for a specific workflow. Always follow the skill's instructions when performing the corresponding task. Invoke one explicitly with `/<name>` in Claude Code or `$<name>` in Codex.

A shared SessionStart hook (`.agents/hooks/session-start.sh`, wired from `.claude/settings.json` for Claude Code and `.codex/hooks.json` for Codex) injects a reminder pointing here at the start of every session and asks the agent to verify that the skills below are loaded (`available skills` in Claude Code's system reminder, `/skills` in Codex).
...
One set of instructions, skills, and hooks serves every agent. Edit the shared files; never fork per tool.

| Purpose | Shared source of truth | Claude Code | Codex CLI |
| --- | --- | --- | --- |
| Project instructions | `AGENTS.md` (this file) | `CLAUDE.md` is a shim that imports it with `@AGENTS.md` | Read natively (`AGENTS.md` at the git root) |
| Skills | `.agents/skills/<name>/SKILL.md` | `.claude/skills` → symlink to `../.agents/skills` | Read natively (`.agents/skills/` at the git root) |
| SessionStart hook | `.agents/hooks/session-start.sh` | `.claude/settings.json` | `.codex/hooks.json` |
| Tool config | — | `.claude/settings.json` | `.codex/config.toml` (raises `project_doc_max_bytes` above this file's size) |

Rules for keeping the two hosts in sync:

- Every `SKILL.md` frontmatter needs both `name` (equal to its directory name, lowercase/hyphens) and `description` (≤ 1024 chars): Codex requires `name`, Claude Code only recommends it. `tests/ts/agent-skills.test.ts` checks the layout, the symlink, the hook JSON, and that the skill list in this file and in the hook script match the directories.
- Add a skill by creating `.agents/skills/<name>/SKILL.md` and adding a row to the table above plus the hook's skill list. Do not put anything under `.claude/skills/` directly — it is a symlink and the file would land in `.agents/skills/` anyway.
- **Codex only applies the project `.codex/` layer (config, hooks) once the project is trusted** — accept the trust prompt on first launch or set `[projects."<absolute path>"] trust_level = "trusted"` in `~/.codex/config.toml`. Skills and `AGENTS.md` load regardless of trust.
- **Windows checkouts need symlink support** (`git config core.symlinks true` with Developer Mode or an elevated shell, then re-checkout) or Claude Code will see `.claude/skills` as a plain text file and load no project skills. Codex is unaffected because it reads `.agents/skills/` directly.
- Keep this file tool-neutral: no `.claude/`-only paths, no Claude- or Codex-specific UI wording outside this section. Keep it under Codex's combined AGENTS.md budget (`project_doc_max_bytes`, raised in `.codex/config.toml`); the vitest guard fails if the file outgrows that setting.
