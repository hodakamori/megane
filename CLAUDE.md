...
- **CI** (`.github/workflows/e2e.yml`) runs the webapp-host and JupyterLab-host projects inside the pinned Playwright container (`mcr.microsoft.com/playwright:v<playwright-version>-noble` — the tag must match the `playwright` devDependency; bump them together) against `tests/e2e/baselines-ci/`. Those baselines are re-recorded **in CI itself** by the "E2E update baselines" `workflow_dispatch` workflow (Actions → E2E update baselines → pick the branch), which commits the PNGs back to the branch — never capture `baselines-ci` PNGs outside that container. A missing CI baseline fails the check (`MEGANE_E2E_REQUIRE_BASELINE=1`), so a PR that adds pixel specs must also dispatch the update workflow. The VSCode-hosted projects (`vscode`, `widget-vscode`) are **not** in CI.
